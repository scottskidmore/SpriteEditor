...
public:
    ...
    /// \brief a animation object
    Animation a;

    /// \brief f pointer to frame object
    Frame* f;
...
private:
    ...
    /// \brief The tools enum stores the different drawing tools
    enum tools {Pen, Eraser, Square, Circle, SprayCan};

    /// \brief currentTool stores the current tool selected
    tools currentTool;
...
public slots:
    ...
    void handleDrawingShapes(QPoint startPoint, QPoint endPoint);
